...
export function BottomNav() {
  ...
  return (
    <nav
      aria-label="Ana menü"
      className="fixed inset-x-0 bottom-0 z-30 mx-auto max-w-md border-t border-white/10 bg-[#24262E]/95 shadow-lg backdrop-blur-xl pb-[env(safe-area-inset-bottom,0px)]"
    >
        ...
        {SEKMELER.map(({ href, label, ikon: Ikon }) => {
          ...
          return (
            <Link
              ...
              className="flex flex-1 flex-col items-center gap-1 py-2.5 text-[11px] font-medium transition active:scale-[0.97] duration-150 ease-out"
            >
              <span
                className={`flex h-9 w-9 items-center justify-center rounded-full transition ${
                  aktif ? "bg-[#6366F1]/20 text-[#818CF8]" : "text-gray-400"
                }`}
              >
                <Ikon className="h-5 w-5" strokeWidth={aktif ? 2.4 : 1.8} aria-hidden="true" />
              </span>
              <span className={aktif ? "text-white" : "text-gray-400"}>{label}</span>
            </Link>
          );
        })}
      </div>
